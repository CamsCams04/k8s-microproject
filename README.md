...
Commande pour tester le fonctionnement :
```
curl --resolve "exemple-virtu.local:443:127.0.0.1" -k -i https://exemple-virtu.local/
```
...
Commande :
```
kubectl create configmap microproject-configmap --from-literal USER_NAME=loricaudin
kubectl exec -it k8s-microproject-deployment-<id> bash
cat out.log
```
